  ...
  return {
    validateProperties: function(newDoc, oldDoc, userContext, securityInfo, docDefinition) {
      ...
      if (newDoc._attachments) {
        storeOptionalValidationErrors(attachmentsValidationModule.validateAttachments(newDoc, docDefinition));
      }

      return validationErrors;

      // The following functions are nested within this function so they can share access to the newDoc, oldDoc and
        ...
        if (!utils.isValueNullOrUndefined(itemValue)) {
          ...
          switch (validatorType) {
            ...
            case 'attachmentReference':
              storeOptionalValidationErrors(attachmentsValidationModule.validateAttachmentReference(newDoc, validator, itemStack));
              break;
            default:
      ...
      function performConditionalValidation(validator) {
        ...
        for (var candidateIndex = 0; candidateIndex < validationCandidates.length; candidateIndex++) {
          ...
          if (typeof candidate.condition === 'function' &&
              candidate.condition(newDoc, oldDoc, currentItemEntry, conditionalValidationItemStack)) {
      ...
      function shouldSkipItemValidation(validator, validatorType) {
        ...
        if (utils.isDocumentMissingOrDeleted(oldDoc)) {
          ...
          return false;
        } else if (utils.isDocumentMissingOrDeleted(newDoc)) {
